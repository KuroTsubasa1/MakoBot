...
const EXECFILES = require('child_process').execFile;
const FS = require('fs');
...
client.on('message', message => {
  ...
  function slots() {
    ...
    if (FS.existsSync(path)) {
      ...
      if (checkRequirements(obj)) {
        obj.coins = Number(obj.coins) + Number(sendSlotResults(slotsLogic(genSlotsNumbers(), 1, obj)));
        ...
        message.channel.send("Sorry but you don't have enough coins.");
      }
    }
  };
  ...
  function wage() {
    ...
    if (FS.existsSync(path)) {
      ...
      obj.coins = Number(obj.coins) + 100;
      FS.writeFile(path, JSON.stringify(obj), function(err) {
        if (err) {
          return console.log(err);
        }
